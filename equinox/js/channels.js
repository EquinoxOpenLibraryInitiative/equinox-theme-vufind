...
VuFind.register("channels", function Channels() {
  function addLinkButtons(elem) {
    ...
    var $cont = $(
      '<div class="dropdown">' +
        ...
        '    <svg width="15" height="10" viewBox="0 0 15 10" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.50219 6.72664L13.2308 0.998047L14.8672 2.63446L7.50219 9.99946L0.137188 2.63446L1.7736 0.998047L7.50219 6.72664Z" fill="white"></path></svg>' +
        "   </button>" +
        "</div>"
    );
      ...
      li.append(
        $("<a/> ", {
          ...
          html:
            ...
            links[i].icon +
            '"></i> ' +
            VuFind.translate(links[i].label),
        })
      );
      $list.append(li);
    }
  ...
  function setupChannelSlider(i, op) {
    ...
    $(op)
      .find(".channel-record")
      .unbind("click")
      .click(function channelRecord(event) {
        ...
        if (!record.data("popover-loaded")) {
          ...
          $.ajax({
            ...
            var newContent =
              ...
              "/Channels/Record?" +
              "id=" +
              encodeURIComponent(record.attr("data-record-id")) +
              "&source=" +
              encodeURIComponent(record.attr("data-record-source")) +
              '" class="btn btn-default">' +
              VuFind.translate("channel_expand") +
              "</a>" +
              '<a href="' +
              record.attr("href") +
              '" class="btn btn-default">' +
              VuFind.translate("View Record") +
              "</a>" +
              "</div>" +
              data;
    ...
    $.ajax(e.target.href).done(function addChannelAjaxDone(data) {
      ...
      $testEls.each(function addRetrievedNonEmptyChannels(i, element) {
        ...
        if (list.children().length === 0) {
          $(
            '.channel-add-menu[data-group="' +
              list.closest(".channel-add-menu").data("group") +
              '"]'
          ).remove();
        }
      });
  ...
  bindChannelAddMenu = function bindChannelAddMenuFunc(iteration, channel) {
    ...
    $(scope)
      .find(".channel-add-menu .dropdown-menu a")
      .click(selectAddedChannel);
    $(scope)
      .find(".channel-add-menu .add-btn")
      .click(function addChannels(e) {
        var links = $(e.target)
